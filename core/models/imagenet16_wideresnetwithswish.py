...
class WideResNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 mean: Union[Tuple[float, ...], float] = IMAGENET16_MEAN,
                 std: Union[Tuple[float, ...], float] = IMAGENET16_STD,
                 padding: int = 0,
...
def wideresnetwithswish(name, dataset='cifar10', num_classes=10, device='cpu'):
    ...
    print (f'IMAGENET16-WideResNet-{depth}-{widen}-{act_fn} uses normalization.')
    if 'imagenet16' in dataset:
        return WideResNet(num_classes=num_classes, depth=depth, width=widen, activation_fn=_ACTIVATION[act_fn], 
                          mean=IMAGENET16_MEAN, std=IMAGENET16_STD)
    else:
